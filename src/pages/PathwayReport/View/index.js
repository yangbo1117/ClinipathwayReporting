...
import { Row, Col, Card, Typography, Space, Button, Drawer, List, Avatar, Radio, Badge } from 'antd';
import { ForkOutlined, FileTextOutlined } from '@ant-design/icons';
import PathwayReportTable from "components/PathwayReportTable/index";
...
class ViewPage extends Component{
    ...
    componentDidMount() {
        ...
        this.props.GetData( id, simpleName );
    }
    ...
    ChangeName = (e) => {
        ...
        this.props.GetData(taskId, value);
    }
    ...
    render() {
        ...
        return (
            <div className="content-card">
                <Card 
                    title={<Row>
                            <Col md={{span:12}} xs={{span:24}} sm={{span:24}}><h3 className='table-name-title'>医疗机构临床路径管理整体信息季度报送表</h3></Col>
                            <Col md={{span:12}} xs={{span:24}} sm={{span:24}}>
                                <div className='flex-end-box'>
                                    <Space>
                                        <Button icon={ <ForkOutlined /> } style={{borderRadius: 15}} onClick={ this.handleHistory }>查看历史版本</Button>
                                        <Radio.Group
                    ...
                    style={{ width: '100%' }}
                >
                    <PathwayReportTable loading={ loading } data={ data } />
                </Card>
                <Drawer
                    ...
                    <List
                        ...
                        renderItem={item => {
                            ...
                            return <List.Item style={{cursor:'pointer'}} onClick={()=>{ this.ChangeHistory(item.id) }}>
                                <List.Item.Meta
                                    avatar={
                                        _.isNull(item.version) ? <Badge.Ribbon text="最新版本" style={{ fontSize: "8px", height: "18px", lineHeight: "18px" }}>
...
export default connect(
    state => ({
        data: state.PathwayReportView.data,
        loading: state.PathwayReportView.loading,
        loadList: state.PathwayReportView.loadList,
        history: state.PathwayReportView.history,
    }),
